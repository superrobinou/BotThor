    ...
    async execute(@SimpleCommandOption("user", { type: SimpleCommandOptionType.User }) user: User, command: SimpleCommandMessage) {
        if (command.message.member.permissions.has("ADMINISTRATOR")) {
            if (user instanceof User) {
                ...
                console.log(member);
                if (member.kickable) {
                    member.roles.add("958056372887502928");
                    const embed = new MessageEmbed()
                        .setTitle("Mute")
                        .setColor("GREEN")
                        .setTimestamp()
                        .setDescription(member.displayName + " a été mute avec succès.")
                    ...
                    const embed = new MessageEmbed()
                        ...
                        .setDescription("Il est impossible de mute ce membre")
                    command.message.reply({ embeds: [embed] });
                }
            }
            else {
                const embed = new MessageEmbed()
                    ...
                    .setColor("RED")
                    .setTimestamp()
                    .setDescription("Un problême est survenue lors du mute, vous n'avez pas ou mal mentionné le membre à mute")
                command.message.reply({ embeds: [embed] });
            }
        }
        else {
            const embed = new MessageEmbed()
                ...
                .setDescription("Vous n'avez pas la permission pour mute un membre")
